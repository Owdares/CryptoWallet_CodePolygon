...
namespace Blaved.Services.BlockChain.Scanners.EthereumSimilar
{
    public class EthereumSimilarBaseScanner
    {
        protected async Task<List<TransactionDTO>> ScanTransaction(Web3 web3, HashSet<string> addresses, HashSet<string> transfersHash,
            BigInteger startBlock, BigInteger lastBlock, bool isToken, int coinDecimal, decimal minAmount, string? contract = null)
        {
            var transactions = isToken 
                ? await ScanTokenTransaction(web3, addresses, transfersHash, startBlock, lastBlock, contract!, coinDecimal, minAmount)
                : await ScanMainTransaction(web3, addresses, transfersHash, startBlock, lastBlock, coinDecimal, minAmount);
